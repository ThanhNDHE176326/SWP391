...
public class BlogListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        CategoryBlogDAO ca = new CategoryBlogDAO();
...
// Pagination
        ...
        if (query != null && !query.trim().isEmpty()) {
            blogs = blogDAO.searchBlogs(query, page, pageSize);
            totalBlogs = blogDAO.getTotalNumberOfBlogs(query);
        } else if (categoryId > 0) {
            blogs = blogDAO.getBlogsByCategory(categoryId, page, pageSize);
            totalBlogs = blogDAO.getTotalNumberOfBlogsByCategory(categoryId);
        } else {
            blogs = blogDAO.getAllBlogs(page, pageSize);
            totalBlogs = blogDAO.getTotalNumberOfBlogs();
        }

        int noOfPages = (int) Math.ceil(totalBlogs * 1.0 / pageSize);

        List<CategoryBlog> categories = ca.getAllCategories();
